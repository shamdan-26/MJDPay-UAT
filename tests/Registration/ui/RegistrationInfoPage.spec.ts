    ...
    test.describe('Tab Panel 1 — Business Info Form [ref_24 – ref_58]', () => {
        ...
        test.describe('Element Visibility', () => {
            ...
            test('should display exactly four Profile Type options', async () => {
                await expect(infoPage.profileTypeGroup.getByRole('radio')).toHaveCount(4);
            });
            ...
            test('should display the Customer radio option [ref_34]', async () => {
                await expect(infoPage.customerCard).toBeVisible();
            });

            test('should display the Customer label and description [ref_35, ref_36]', async () => {
                await expect(infoPage.customerCard.locator('.mp-rc-title')).toContainText('Customer');
                await expect(infoPage.customerCard.locator('.mp-rc-sub')).toContainText('Accept payments and manage your store.');
            });
            ...
            test('should display the Freelancer label and description [ref_38, ref_39]', async () => {
                ...
                await expect(infoPage.freelancerCard.locator('.mp-rc-sub')).toContainText('Accept payments and manage your store.');
            });
            ...
            test('should display the Unified Number textbox with correct placeholder [ref_43]', async () => {
                await expect(infoPage.crnInput).toHaveAttribute('placeholder', 'Eg. 1023456789');
            });
        ...
        test.describe('Field Interactions', () => {
            ...
            test('should allow selecting Customer profile type [ref_34]', async () => {
                await infoPage.customerCard.click();
                await expect(infoPage.customerCard).toHaveAttribute('aria-checked', 'true');
            });

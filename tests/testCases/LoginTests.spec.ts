...
const loginData = require('../../test-data/LoginTests.json');
...
test.describe('MajdPay Login Tests', () => {
    ...
    test.describe('Successful Login Scenarios', () => {
        ...
        for (const data of positiveTests) {
            test(`Login for user: ${data.description}`, async ({ loginPage }) => {
    ...
    test.describe('Validation / Negative Login Scenarios', () => {
        ...
        for (const data of negativeTests) {
            test(`Validation: ${data.description}`, async ({ loginPage, page }) => {
                if (data.execute === false) {
                    test.skip();
                }
    ...
    test.describe('UI Theme and Language Login Scenarios', () => {
        ...
        test('Login in Light Mode (Default)', async ({ loginPage }) => {
        ...
        test('Login in Dark Mode', async ({ loginPage }) => {
            if (creds.execute === false) {
                test.skip();
            }
        ...
        test('Login in English Language (Default)', async ({ loginPage }) => {
            if (creds.execute === false) {
                test.skip();
            }
            ...
            // Step 2: Confirm English is the active language BEFORE login (switch if not)
            let isEnglishActive = await loginPage.isLanguageActive(loginPage.englishButton);
            if (!isEnglishActive) {
                await loginPage.selectEnglish();
                isEnglishActive = await loginPage.isLanguageActive(loginPage.englishButton);
            }
    ...
    test.describe('Password Visibility Scenarios', () => {
        ...
        test('Merchant - Verify that the text inside the password field is masked by default', async ({ loginPage }) => {
            if (creds.execute === false) {
                test.skip();
            }
        ...
        test('Merchant - Show the password', async ({ loginPage }) => {
            if (creds.execute === false) {
                test.skip();
            }
        ...
        test('Merchant - Hide the password', async ({ loginPage }) => {
            if (creds.execute === false) {
                test.skip();
            }

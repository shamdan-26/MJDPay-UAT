...
import { goToInfoStep, REGISTER_URL, nextResidentAsset, generateEmail } from '../RegistrationHelper';
...
async function navigateToFinancialTab(browser: any): Promise<Page> {
    ...
    await page.getByRole('button', { name: /Loading|جاري التحميل/i })
        ...
        .catch(() => {});
    await page.getByRole('textbox', { name: /monthly expected number|العدد الشهري المتوقع للفواتير/i })

...
test.describe('Registration – Financial & Business Step (Tab 2 of 3)', () => {
    ...
    test('should display the Banks dropdown label', async ({ page }) => {
        ...
        await expect(page.locator('.floating-field-label.ng-star-inserted', { hasText: /industries/i })).toBeVisible();
        await selectRandomOption(page, page.locator('[id^="floating-dropdown-industries"]'));

        await expect(page.locator('.floating-field-label.ng-star-inserted', { hasText: /annual income/i })).toBeVisible();
        await selectRandomOption(page, page.locator('[id^="floating-dropdown-annual-income"]'));
    ...
    test('should display the Annual Income dropdown', async ({ page }) => {
        await expect(page.locator('#floating-dropdown-annual-income-10.mat-mdc-select.mat-mdc-select-empty')).toBeVisible();
    });
    ...
    test('should open the Annual Income dropdown when clicked', async ({ page }) => {
        await page.locator('#floating-dropdown-annual-income-10.mat-mdc-select.mat-mdc-select-empty').click();
        await expect(page.locator('mat-option').first()).toBeVisible({ timeout: 5000 });
    });
    ...
    test('should proceed to Verification & Uploads tab when Next is clicked with valid data', async ({ page }) => {
        ...
        await expect(page.locator('#register-form-title.form-title'))
            .toContainText(/verification & documents/i, { timeout: 10000 });
    });
